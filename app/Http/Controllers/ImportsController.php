    ...
    protected function buildMappingPreview(string $stored, ?array $selectedMapping = null): ?array
    {
        ...
        foreach ($headers as $idx => $header) {
            ...
            $autoMap[$idx] = $selectedMapping[$idx] ?? match (true) {
                ...
                str_contains($lower, 'country') || $lower === 'country code' => 'phone_country',
                ...
                str_contains($lower, 'gender') || str_contains($lower, 'sex') => 'gender',
                str_contains($lower, 'rate') || str_contains($lower, 'star') => 'rating',
                str_contains($lower, 'lifecycle') || $lower === 'stage' || $lower === 'life cycle' => 'lifecycle_stage',
                str_contains($lower, 'status') && ! str_contains($lower, 'facebook') => 'status',
                str_contains($lower, 'comment') || str_contains($lower, 'admin') => 'admin_comment',
                str_contains($lower, 'fb') || str_contains($lower, 'facebook') => 'facebook',
                str_contains($lower, 'twit') || $lower === 'x' => 'twitter',
                str_contains($lower, 'link') || $lower === 'in' => 'linkedin',
