...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div class="lg:col-span-2 space-y-4">
            ...
            <x-ui.card-content class="space-y-4">
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div class="space-y-1 flex-1 min-w-0">
                        ...
                        <input id="photo" name="photo" type="file" accept="image/*" {{ !$canEdit ? 'disabled' : '' }}
        ...
        <x-ui.card>
            ...
            <x-ui.card-content x-data="htmlEditor('description_html')">
                ...
                <div class="flex flex-wrap gap-1 mb-2 p-1 rounded-md border border-input bg-muted/30">
                    <button type="button" @click="exec('bold')" title="Bold" class="h-7 w-7 grid place-items-center rounded text-sm font-bold hover:bg-accent disabled:opacity-50 disabled:cursor-not-allowed" {{ !$canEdit ? 'disabled' : '' }}>B</button>
                    <button type="button" @click="exec('italic')" title="Italic" class="h-7 w-7 grid place-items-center rounded text-sm italic hover:bg-accent disabled:opacity-50 disabled:cursor-not-allowed" {{ !$canEdit ? 'disabled' : '' }}>I</button>
                    <button type="button" @click="exec('underline')" title="Underline" class="h-7 w-7 grid place-items-center rounded text-sm underline hover:bg-accent disabled:opacity-50 disabled:cursor-not-allowed" {{ !$canEdit ? 'disabled' : '' }}>U</button>
                    <div class="w-px bg-border mx-1"></div>
                    <button type="button" @click="exec('insertUnorderedList')" title="Bullet list" class="h-7 w-7 grid place-items-center rounded hover:bg-accent text-xs disabled:opacity-50 disabled:cursor-not-allowed" {{ !$canEdit ? 'disabled' : '' }}>• —</button>
                    <button type="button" @click="exec('insertOrderedList')" title="Numbered list" class="h-7 w-7 grid place-items-center rounded hover:bg-accent text-xs disabled:opacity-50 disabled:cursor-not-allowed" {{ !$canEdit ? 'disabled' : '' }}>1.</button>
                    <div class="w-px bg-border mx-1"></div>
                    <button type="button" @click="exec('removeFormat')" title="Clear formatting" class="h-7 px-2 rounded hover:bg-accent text-xs text-muted-foreground disabled:opacity-50 disabled:cursor-not-allowed" {{ !$canEdit ? 'disabled' : '' }}>Clear</button>
                </div>
                {{-- Editor --}}
                <div x-ref="editor"
        ...
        <div x-data='{
            ...
            add() { if (!{{ $canEdit ? 'true' : 'false' }}) return; this.rows.push({ key: "", value: "" }); this.$nextTick(() => this.$refs.list?.lastElementChild?.querySelector("input")?.focus()); },
            remove(i) { if (!{{ $canEdit ? 'true' : 'false' }}) return; this.rows.splice(i, 1); }
        }'>
            <x-ui.card>
                <x-ui.card-header class="flex flex-row items-center justify-between">
                    ...
                    <button type="button" @click="add" class="inline-flex items-center gap-1 h-7 px-2 rounded-md text-xs font-medium border border-input bg-background hover:bg-accent disabled:opacity-50 disabled:cursor-not-allowed" {{ !$canEdit ? 'disabled' : '' }}>
                    ...
                    <div x-ref="list" class="space-y-2">
                        <template x-for="(row, i) in rows" :key="i">
                            <div class="flex flex-wrap gap-2 items-start">
                                <input type="text" name="custom_fields_keys[]" x-model="row.key" placeholder="Label" {{ !$canEdit ? 'disabled' : '' }}
                                ...
                                <input type="text" name="custom_fields_values[]" x-model="row.value" placeholder="Value" {{ !$canEdit ? 'disabled' : '' }}
                                ...
                                <button type="button" @click="remove(i)" class="h-9 w-9 grid place-items-center rounded-md border border-input text-muted-foreground hover:text-destructive hover:border-destructive disabled:opacity-50 disabled:cursor-not-allowed" {{ !$canEdit ? 'disabled' : '' }}>
    ...
    <div class="space-y-4">
        ...
        <x-ui.card x-data="{
            ...
            hover: 0,
            set(v) { if (!{{ $canEdit ? 'true' : 'false' }}) return; this.rating = v; document.getElementById('rating-input').value = v; }
        }">
            <x-ui.card-header>
                ...
                <div class="flex items-center gap-1">
                    <template x-for="i in 5" :key="i">
                        <button type="button"
                                ...
                                @mouseenter="hover = i"
                                @mouseleave="hover = 0"
                                class="h-8 w-8 grid place-items-center transition-colors {{ !$canEdit ? 'opacity-50 cursor-not-allowed' : 'hover:scale-110' }}">
        ...
        <x-ui.card x-data="{
            ...
            apply() { if (!{{ $canEdit ? 'true' : 'false' }}) return; this.suggested.forEach(id => { const cb = document.querySelector(`input[name='tags[]'][value='${id}']`); if (cb && !cb.checked) cb.checked = true; }); window.dispatchEvent(new CustomEvent('toast', { detail: { type: 'success', message: 'Suggested tags applied.' }})); },
            isSuggested(id) { return this.suggested.includes(String(id)); }
        }">
            <x-ui.card-header class="flex flex-row items-center justify-between">
                <x-ui.card-title>Tags</x-ui.card-title>
                @if ($tags->isNotEmpty())
                    <button type="button" @click="suggest" :disabled="busy || !{{ $canEdit ? 'true' : 'false' }}" class="inline-flex items-center gap-1 h-7 px-2 rounded-md text-xs font-medium border border-input bg-background hover:bg-accent disabled:opacity-50">
                    ...
                    <div class="flex flex-wrap gap-2">
                        @foreach ($tags as $tag)
                            <label class="cursor-pointer relative {{ !$canEdit ? 'opacity-50 pointer-events-none' : '' }}">
                                <input type="checkbox" name="tags[]" value="{{ $tag->id }}" class="peer sr-only" @checked(in_array($tag->id, $selectedTagIds)) {{ !$canEdit ? 'disabled' : '' }} />
